      ...
      >
        ...
        {step === 'intro' && (
          <div style={{ padding: '36px 32px 32px', display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
              <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#b0acbe', transition: 'background 150ms' }}
            ...
            <div style={{ fontFamily: 'Hanken Grotesk, sans-serif', fontSize: 22, fontWeight: 700, color: '#1c1b22', letterSpacing: '-0.02em', marginBottom: 8 }}>
            ...
            <button
              ...
              style={{ width: '100%', background: '#5e4dbb', color: '#fff', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: 14, fontWeight: 600, padding: '13px 0', borderRadius: 12, border: 'none', cursor: 'pointer', transition: 'background 150ms', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          ...
          <div style={{ padding: '28px 32px 32px', display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                {['scan', 'verify'].map((s, i) => (
                  <div key={s} style={{ width: i === 0 ? 20 : 8, height: 8, borderRadius: 4, background: i === 0 ? '#5e4dbb' : '#e8e4f0', transition: 'all 300ms' }} />
                ))}
              </div>
            ...
            <div style={{ fontFamily: 'Hanken Grotesk, sans-serif', fontSize: 20, fontWeight: 700, color: '#1c1b22', letterSpacing: '-0.01em', marginBottom: 6 }}>
              Scan the QR code
            </div>
            ...
            {secret && (
              <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 10, padding: '10px 12px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                <div>
                  ...
                  <div style={{ fontFamily: 'monospace', fontSize: 13, color: '#1c1b22', letterSpacing: '0.1em', wordBreak: 'break-all' as const }}>
            ...
            <button
              ...
              style={{ width: '100%', background: loadingSetup || !qrCode ? '#c9c4d5' : '#5e4dbb', color: '#fff', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: 14, fontWeight: 600, padding: '13px 0', borderRadius: 12, border: 'none', cursor: loadingSetup || !qrCode ? 'not-allowed' : 'pointer', transition: 'background 150ms', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
              onMouseEnter={e => { if (!loadingSetup && qrCode) e.currentTarget.style.background = '#4f3fa8'; }}
              onMouseLeave={e => { if (!loadingSetup && qrCode) e.currentTarget.style.background = '#5e4dbb'; }}
            >
              I've scanned it
              <Icon name="arrow_forward" size={16} color="#fff" />
            </button>
        ...
        {step === 'verify' && (
          <div style={{ padding: '28px 32px 32px', display: 'flex', flexDirection: 'column', gap: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                {['scan', 'verify'].map((s, i) => (
                  <div key={s} style={{ width: i === 1 ? 20 : 8, height: 8, borderRadius: 4, background: i === 1 ? '#5e4dbb' : '#e8e4f0', transition: 'all 300ms' }} />
                ))}
              </div>
              <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, border: 'none', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 150ms' }}
                onMouseEnter={e => { e.currentTarget.style.background = '#f1ecf6'; }}
            ...
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: '#787584', lineHeight: 1.5, marginBottom: 28 }}>
              ...
              {otp.map((digit, i) => (
                <input
                  ...
                  style={{
                    ...
                    border: `2px solid ${otpError ? '#ffdad6' : digit ? '#5e4dbb' : '#E5E7EB'}`,
            ...
            {otpError && (
              <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#ba1a1a', textAlign: 'center', marginBottom: 16, marginTop: 4 }}>{otpError}</div>
            )}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                onMouseEnter={e => { if (!verifying && otpComplete) e.currentTarget.style.background = '#4f3fa8'; }}
                onMouseLeave={e => { if (!verifying && otpComplete) e.currentTarget.style.background = '#5e4dbb'; }}
              >
                {verifying ? 'Activating…' : (
                  <><Icon name="shield_lock" size={15} color="#fff" />Activate 2FA</>
                )}
              </button>
            </div>
          </div>
        ...
        {step === 'done' && (
          <div style={{ padding: '48px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, color: '#787584', textAlign: 'center', lineHeight: 1.5 }}>
