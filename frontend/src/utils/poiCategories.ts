...
export function createPoiDivIcon(category: PoiCategory, size = 28): string {
  ...
  return `<div style="width:${size}px;height:${size}px;border-radius:${Math.round(size * 0.3)}px;background:${cfg.bg};border:1.5px solid ${cfg.borderColor};display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.18);cursor:pointer;font-family:'Material Symbols Outlined';font-size:${Math.round(size * 0.56)}px;color:${cfg.fg};line-height:1;font-variation-settings:'FILL' 1,'wght' 400;">${cfg.icon}</div>`;
}
...
export function createPinDivIcon(sym: string, highlighted: boolean, size = 30): string {
  ...
  const starBadge = highlighted
    ? `<div style="position:absolute;top:-5px;right:-5px;width:14px;height:14px;border-radius:50%;background:#5e4dbb;border:2px solid #fff;display:flex;align-items:center;justify-content:center;font-family:'Material Symbols Outlined';font-size:8px;color:#fff;font-variation-settings:'FILL' 1,'wght' 400;line-height:1;">star</div>`
    : '';
  return `<div style="position:relative;width:${size}px;height:${size}px;"><div style="width:${size}px;height:${size}px;border-radius:${Math.round(size * 0.3)}px;background:${bg};border:2px solid ${border};display:flex;align-items:center;justify-content:center;box-shadow:0 3px 10px rgba(94,77,187,0.30);cursor:pointer;font-family:'Material Symbols Outlined';font-size:${Math.round(size * 0.53)}px;color:${fg};line-height:1;font-variation-settings:'FILL' 1,'wght' 400;">${icon}</div>${starBadge}</div>`;
}
