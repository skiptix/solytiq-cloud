...
function StatCard({ num, label, sub, icon, iconBg, iconColor, accent, onClick }: { num: number; label: string; sub: string; icon: string; iconBg: string; iconColor: string; accent?: string; onClick?: () => void }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={onClick} role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined}
      ...
      style={{ background: 'var(--color-surface-gray)', border: `1px solid ${hov ? 'var(--color-purple-tint-2)' : 'var(--color-border-alt)'}`, borderRadius: 12, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10, transition: 'all 180ms', minWidth: 0, cursor: onClick ? 'pointer' : 'default', boxShadow: hov && onClick ? '0 6px 16px rgba(var(--color-primary-rgb), 0.12)' : 'none', transform: hov && onClick ? 'translateY(-2px)' : 'translateY(0)' }}>
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={e => onOpen(task, e)}
      ...
      <div onClick={e => { e.stopPropagation(); onToggle(task.id); }}
        style={{ width: 18, height: 18, minWidth: 18, borderRadius: 5, border: '1.5px solid', borderColor: task.checked ? 'var(--color-primary)' : 'var(--color-border-strong)', background: task.checked ? 'var(--color-primary)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 150ms', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--color-text-primary)', opacity: task.checked ? 0.4 : 1, textDecoration: task.checked ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{task.title}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
          {task._source === 'list' && task._listName && <span style={{ fontFamily: 'var(--font-body)', fontSize: 10.5, color: 'var(--color-text-tertiary)' }}>· {task._listName}</span>}
          {task.priority && <span style={{ fontFamily: 'var(--font-body)', fontSize: 10.5, fontWeight: 600, color: PCOLS[task.priority] }}>{task.priority}</span>}
        </div>
      </div>
      <span style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--color-text-tertiary)', flexShrink: 0 }}>{friendlyDate(task.deadline, timezone)}</span>
    </div>
  );
}
...
function ObserverPanel({ title, icon, accent, accentBg, tasks, emptyText, onToggle, onOpen, onSeeMore, timezone }: { title: string; icon: string; accent: string; accentBg: string; tasks: Task[]; emptyText: string; onToggle: (id: number) => void; onOpen: (t: Task, e: React.MouseEvent) => void; onSeeMore: () => void; timezone: string }) {
  ...
  return (
    <div style={{ background: 'var(--color-surface-gray)', border: '1px solid var(--color-border-alt)', borderRadius: 14, padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0 }}>
...
function TasksDetailModal({ title, icon, accent, accentBg, tasks, onClose, onToggle, onDelete, onUpdate, onRowClick }: { title: string; icon: string; accent: string; accentBg: string; tasks: Task[]; onClose: () => void; onToggle: (id: number) => void; onDelete: (id: number) => void; onUpdate: (id: number, u: Partial<Task>) => void; onRowClick: (t: Task, e: React.MouseEvent) => void }) {
  ...
  const pill = (key: typeof filter, label: string) => (
    <button onClick={() => setFilter(key)} style={{ fontFamily: 'var(--font-heading)', fontSize: 12, fontWeight: 600, border: 'none', borderRadius: 9999, padding: '5px 12px', cursor: 'pointer', background: filter === key ? 'var(--color-primary)' : 'var(--color-surface-tint-2)', color: filter === key ? 'var(--color-white)' : 'var(--color-text-tertiary)' }}>{label}</button>
  );
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(var(--color-black-rgb), 0.22)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
      ...
      <div style={{ background: 'var(--color-white)', borderRadius: 16, width: '100%', maxWidth: 620, maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 12px 40px rgba(var(--color-black-rgb), 0.18)', animation: 'modalIn 280ms cubic-bezier(0.34,1.56,0.64,1) both', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '18px 20px', borderBottom: '1px solid var(--color-surface-tint-2)', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 17, fontWeight: 700, color: 'var(--color-text-primary)' }}>{title}</div>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--color-text-tertiary)', marginTop: 2 }}>{tasks.length} total · {tasks.filter(t => !t.checked).length} open</div>
          </div>
        ...
        <div style={{ padding: '14px 20px 8px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <select value={sort ?? ''} onChange={e => setSort(e.target.value || null)}
              style={{ fontFamily: 'var(--font-heading)', fontSize: 12, border: '1.5px solid var(--color-border)', borderRadius: 8, padding: '4px 8px', background: 'var(--color-white)', color: 'var(--color-text-tertiary)', cursor: 'pointer' }}>
...
export default function DashboardScreen() {
  ...
  const pill = (key: string, label: string) => (
    <button onClick={() => setFilter(key)} style={{ fontFamily: 'var(--font-heading)', fontSize: 11, fontWeight: 600, border: 'none', borderRadius: 9999, padding: '4px 10px', cursor: 'pointer', background: filter === key ? 'var(--color-primary)' : 'var(--color-surface-tint-2)', color: filter === key ? 'var(--color-white)' : 'var(--color-text-tertiary)' }}>{label}</button>
  );
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: isMobile ? '20px 16px 80px' : '32px 32px 48px', display: 'flex', flexDirection: 'column', gap: 24, width: '100%' }}>
        ...
        <header style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 28, fontWeight: 700, color: 'var(--color-text-primary)', letterSpacing: '-0.02em', lineHeight: 1.15 }}>Dashboard</h1>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 13.5, color: 'var(--color-text-tertiary)', marginTop: 6 }}>
              {dueTodayTasks.length > 0 ? <>You have <strong style={{ color: 'var(--color-primary)' }}>{dueTodayTasks.length} task{dueTodayTasks.length === 1 ? '' : 's'}</strong> due today{dueWeekTasks.length > 0 && <> and <strong style={{ color: 'var(--color-text-primary)' }}>{dueWeekTasks.length}</strong> more this week</>}.</> : dueWeekTasks.length > 0 ? <>Nothing due today — <strong style={{ color: 'var(--color-text-primary)' }}>{dueWeekTasks.length}</strong> task{dueWeekTasks.length === 1 ? '' : 's'} ahead this week.</> : 'All clear. No deadlines on the horizon.'}
            </div>
          </div>
          {overdueTasks.length > 0 && (
            <div style={{ background: 'var(--color-error-bg-alt)', border: '1px solid var(--color-error-bg)', borderRadius: 9999, padding: '6px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontFamily: 'var(--font-heading)', fontSize: 12, fontWeight: 600, color: 'var(--color-error)' }}>{overdueTasks.length} overdue</span>
            </div>
          )}
        </header>

        <UpcomingTimelineWidget accent="var(--color-primary)" />

        <section style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 12 }}>
          <StatCard num={openCount} label="Open Tasks" sub={`${totalCount} total`} icon="inventory_2" iconBg="var(--color-surface-tint)" iconColor="var(--color-primary)"
            onClick={() => setDetailModal({ source: 'open', title: 'Open Tasks', icon: 'inventory_2', accent: 'var(--color-primary)', accentBg: 'var(--color-surface-tint)' })} />
          <StatCard num={completedCount} label="Completed" sub={completedCount > 0 ? `${pct}%` : 'Get started'} icon="check_circle" iconBg="rgba(var(--color-success-rgb), 0.10)" iconColor="var(--color-success)" accent="var(--color-success)"
            onClick={() => setDetailModal({ source: 'completed', title: 'Completed', icon: 'check_circle', accent: 'var(--color-success)', accentBg: 'rgba(var(--color-success-rgb), 0.10)' })} />
          <StatCard num={dueTodayTasks.length} label="Due Today" sub={dueTodayTasks.length > 0 ? 'Focus' : 'Clear'} icon="today" iconBg="var(--color-orange-pale-3)" iconColor="var(--color-orange)" accent={dueTodayTasks.length > 0 ? 'var(--color-orange)' : undefined}
            onClick={() => setDetailModal({ source: 'today', title: 'Due Today', icon: 'today', accent: 'var(--color-orange)', accentBg: 'var(--color-orange-pale-3)' })} />
          <StatCard num={dueWeekTasks.length} label="Due This Week" sub="Upcoming" icon="calendar_month" iconBg="var(--color-blue-pale-2)" iconColor="var(--color-blue-mid-7)"
            onClick={() => setDetailModal({ source: 'week', title: 'Due This Week', icon: 'calendar_month', accent: 'var(--color-blue-mid-7)', accentBg: 'var(--color-blue-pale-2)' })} />
        </section>

        <section style={{ background: 'var(--color-surface-gray)', border: '1px solid var(--color-border-alt)', borderRadius: 12, padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 12, fontWeight: 600, color: 'var(--color-text-secondary)' }}>This week's progress</div>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 12, color: 'var(--color-text-tertiary)' }}><strong style={{ color: 'var(--color-text-primary)' }}>{completedCount}</strong> done · <strong style={{ color: 'var(--color-text-primary)' }}>{openCount}</strong> open</div>
          </div>
          <div style={{ height: 8, background: 'var(--color-purple-pale-39)', borderRadius: 9999, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${pct}%`, background: pct === 100 ? 'var(--color-success)' : 'linear-gradient(90deg,var(--color-accent-purple-light) 0%,var(--color-primary) 100%)', borderRadius: 9999, transition: 'width 600ms ease-in-out' }} />
          </div>
        </section>

        <section style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 14 }}>
          <ObserverPanel title="Due Today" icon="today" accent="var(--color-orange)" accentBg="var(--color-orange-pale-3)" tasks={dueTodayTasks} emptyText="Nothing on your plate today." onToggle={toggle} onOpen={openTask} timezone={timezone} onSeeMore={() => setDetailModal({ source: 'today', title: 'Due Today', icon: 'today', accent: 'var(--color-orange)', accentBg: 'var(--color-orange-pale-3)' })} />
          <ObserverPanel title="This Week" icon="calendar_month" accent="var(--color-blue-mid-7)" accentBg="var(--color-blue-pale-2)" tasks={dueWeekTasks} emptyText="No deadlines this week." onToggle={toggle} onOpen={openTask} timezone={timezone} onSeeMore={() => setDetailModal({ source: 'week', title: 'Due This Week', icon: 'calendar_month', accent: 'var(--color-blue-mid-7)', accentBg: 'var(--color-blue-pale-2)' })} />
        </section>

        <section>
          <div style={{ background: 'var(--color-surface-gray)', border: '1px solid var(--color-border-alt)', borderRadius: 12, overflow: 'hidden' }}>
        ...
        <section>
          <div style={{ background: 'var(--color-surface-gray)', border: '1px solid var(--color-border-alt)', borderRadius: 12, padding: '16px 12px 8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, padding: '0 4px', gap: 8, flexWrap: 'wrap' }}>
              <div style={{ fontFamily: 'var(--font-heading)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-quaternary)' }}>Todos</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                ...
                {lists.length > 0 && (
                  <select value={!['all','local'].includes(filter) ? filter : ''} onChange={e => { if (e.target.value) setFilter(e.target.value); }}
                    style={{ fontFamily: 'var(--font-heading)', fontSize: 11, fontWeight: 600, border: 'none', borderRadius: 9999, padding: '4px 10px', cursor: 'pointer', background: !['all','local'].includes(filter) ? 'var(--color-primary)' : 'var(--color-surface-tint-2)', color: !['all','local'].includes(filter) ? 'var(--color-white)' : 'var(--color-text-tertiary)' }}>
                    <option value="">To-Do ▾</option>
                    {lists.map(l => <option key={l.id} value={l.id}>{l.emoji} {l.name}</option>)}
                  </select>
                )}
                <select value={sort ?? ''} onChange={e => setSort(e.target.value || null)}
                  style={{ fontFamily: 'var(--font-heading)', fontSize: 11, border: '1.5px solid var(--color-border)', borderRadius: 8, padding: '4px 8px', background: sort ? 'var(--color-surface-tint)' : 'var(--color-white)', color: sort ? 'var(--color-primary)' : 'var(--color-text-tertiary)', cursor: 'pointer' }}>
                  <option value="">Sort</option>
                  <option value="date-asc">Date ↑</option>
                  <option value="date-desc">Date ↓</option>
                  <option value="name-az">Name A-Z</option>
            ...
            {visibleTasks.length === 0 ? (
              <div style={{ padding: '16px 10px', fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--color-text-quaternary)', textAlign: 'center' }}>No open tasks here yet.</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {visibleTasks.length > 5 && (
                  <button onClick={() => setDetailModal({ source: 'todos', title: 'All Todos', icon: 'inventory_2', accent: 'var(--color-primary)', accentBg: 'var(--color-surface-tint)' })}
                    ...
                    style={{ marginTop: 6, width: '100%', background: 'transparent', border: '1px dashed var(--color-purple-tint-2)', borderRadius: 8, padding: '9px 10px', cursor: 'pointer', fontFamily: 'var(--font-heading)', fontSize: 12.5, fontWeight: 600, color: 'var(--color-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 160ms' }}>
                    + {visibleTasks.length - 5} more <Icon name="chevron_right" size={12} color="var(--color-primary)" />
                  </button>
                )}
              </div>
            )}
