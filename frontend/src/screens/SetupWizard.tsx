...
const w: Record<string, CSSProperties> = {
  ...
  card: { width: '100%', maxWidth: 460, background: '#fff', border: '1px solid #E5E7EB', borderRadius: 16, padding: '32px 36px 28px', boxShadow: '0 1px 2px rgba(0,0,0,0.05), 0 8px 32px rgba(94,77,187,0.06)', display: 'flex', flexDirection: 'column', gap: 24 },
  ...
  input: { fontFamily: 'Inter, sans-serif', fontSize: 14, color: '#1c1b22', background: '#fafaff', border: '1.5px solid #ececf3', borderRadius: 10, padding: '13px 16px', outline: 'none', width: '100%', transition: 'all 180ms' },
...
function PasswordStrength({ password }: { password: string }) {
  const { score, label, color } = (() => {
    ...
    return { score: s <= 4 ? 3 : 4, label: s <= 4 ? 'Strong' : 'Very Strong', color: '#10B981' };
  })();
  return (
  ...
  const stepContent = () => {
        ...
        {[['person', 'Pick a username'], ['mail', 'Add your email'], ['lock', 'Choose a secure password']].map(([icon, text]) => (
          <>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: '#F5F3FF', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Icon name={icon} size={16} color="#5e4dbb" />
            </div>
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13.5, color: '#1c1b22' }}>{text}</div>
          </>
        ))}
      </div>
    );
    if (step === 1) return fieldGroup(<>
      ...
      <div style={{ position: 'relative' }}>
        <span style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', fontFamily: 'Inter, sans-serif', fontSize: 14, color: '#b0acbe', pointerEvents: 'none' }}>@</span>
        <input autoFocus value={username} onChange={e => setUsername(e.target.value)} onKeyDown={e => e.key === 'Enter' && goNext()} placeholder="alex_admin"
          style={{ ...w.input, paddingLeft: 40, borderColor: username && !usernameValid ? '#ba1a1a' : '#ececf3' }} />
      </div>
      <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: username && !usernameValid ? '#ba1a1a' : '#787584' }}>3–20 characters · letters, numbers, underscores</div>
    </>);
    if (step === 2) return fieldGroup(<>
      ...
      <input autoFocus type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && goNext()} placeholder="you@example.com"
        style={{ ...w.input, borderColor: email && !emailValid ? '#ba1a1a' : '#ececf3' }} />
      {email && !emailValid && <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#ba1a1a' }}>Enter a valid email address.</div>}
    </>);
    if (step === 3) return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {fieldGroup(<>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button type="button" onClick={() => setShowPw(v => !v)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'Hanken Grotesk, sans-serif', fontSize: 11.5, fontWeight: 600, color: '#5e4dbb' }}>{showPw ? 'Hide' : 'Show'}</button>
          </div>
          <input autoFocus type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••"
            style={{ ...w.input, borderColor: password && !passwordValid ? '#ba1a1a' : '#ececf3' }} />
        ...
        {fieldGroup(<>
          ...
          <input type={showPw ? 'text' : 'password'} value={confirm} onChange={e => setConfirm(e.target.value)} onKeyDown={e => e.key === 'Enter' && goNext()} placeholder="••••••••"
            style={{ ...w.input, borderColor: confirm && !confirmValid ? '#ba1a1a' : '#ececf3' }} />
          {confirm && !confirmValid && <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#ba1a1a' }}>Passwords don't match.</div>}
        </>)}
      </div>
    );
    if (step === 4) return (
  ...
  return (
    <div style={w.wrap}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
        <img src="/assets/solytiq-todo-logo.png" alt="Solytiq Cloud" style={{ width: 64, height: 64, borderRadius: 16, objectFit: 'cover', boxShadow: '0 4px 20px rgba(94,77,187,0.18)' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: step === 0 ? 'center' : 'space-between' }}>
            {step > 0 && (
              <button onClick={() => setStep(s => s - 1)}
                style={{ fontFamily: 'Hanken Grotesk, sans-serif', fontSize: 13, fontWeight: 500, color: '#484552', background: 'transparent', border: 'none', padding: '10px 6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
