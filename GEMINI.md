...
- **Markdown pages get real editing tools, not just text generation.** Six tools live in the shared AI tool registry (`aiTools.ts`) alongside everything else: `list_markdown_lists`, `get_markdown_list` (returns every block with its id and type), and `add_markdown_block`/`update_markdown_block`/`remove_markdown_block`/`move_markdown_block` — one per `/` slash command (heading, paragraph, bulleted-list-item, numbered-list-item, todo, quote, divider, link; image blocks can be reordered/re-captioned but never created by AI, since that requires a real file upload). All four mutating tools funnel through **`mutateMarkdownListBlocks()`** (`routes/markdownLists.ts`) — the same lock → reconcile-todos → prune-images → persist sequence the manual-edit `PUT` route's content branch uses, so a `/todo` block the AI adds mirrors into the auto-managed Todo list exactly like one typed by hand.
- **`MarkdownListAIAssist.tsx`** (`components/AIAssistant/`) is the "Ask AI" button + popover in `MarkdownListScreen.tsx`'s header. Unlike Sol, it only ever sends the model those 5 markdown tools (filtered from `GET /api/ai/tools` by name) and runs its own short tool-calling loop against `POST /api/ai/execute`, capped at 15 rounds. **Scoping is enforced client-side, not just by convention:** every tool call's `markdown_list_id` argument is force-overwritten with the currently-open document's id before execution, so the assistant can never touch a different page even if it hallucinates one. After any round that mutated the doc, the panel re-fetches it (`useMarkdownListsStore.getDetail`) and pushes the fresh blocks back into `MarkdownListScreen`'s local state, so edits appear live without a manual reload.
