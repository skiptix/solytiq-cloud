...
- 🧩 **Discover Apps** — An admin-installable plugin system to enable optional modules like GPS Routes and File Sharing on demand.
- 🗺️ **GPS Tracks & Routing** — Upload, analyze, and map GPX/FIT files directly within your workspace. (Available via Discover Apps)
...
- 📎 **Cloud File Sharing** — Securely share files (max upload size: 200 MB, Nginx proxy limit: 210 MB) with password protection, expiry dates, and public links. (Available via Discover Apps)
...
- **Apps Registry** — Optional features (like GPS Routes and File Sharing) are hidden until an admin installs them via the "Discover Apps" dialog (Settings → System).
